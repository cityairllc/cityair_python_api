...
class TestDeviceData(CityAirApiTestCase):

    def setup(self):
        self.serial = random.choice(self.r.get_devices())


class TestOnlineDeviceData(TestDeviceData):
    ...
    def test_first_packet(self):
        df = self.r.get_device_data(self.serial, take_count=5, last_packet_id=0, all_cols=True)
